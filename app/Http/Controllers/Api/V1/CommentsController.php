...
{ 
    ...
    public function innerComments(Request $request)
    {
        // Validate the incoming request
        $request->validate([
            "post_id" => "required|regex:/^@\w+$/",
            "comment_id" => "required",
            "comments" => "required",
            "comments_vid_path" => "nullable|mimes:mp4,avi,mov,wmv,flv|max:20480", 
            'comments_img_path' => 'array',
            'comments_img_path.*' => "nullable|image|max:2048",
            "comments_link" => "nullable|url", 
        ]);
            ...
            foreach ($request->file('comments_img_path') as $file) {
                ...
                if ($file->isValid()) {
                    ...
                    $uploadCloudinary = cloudinary()->upload(
                        $file->getRealPath(),
                        [
                            'folder' => 'africtv/inner_comment_images',
                            'resource_type' => 'auto',
                            'transformation' => [
                                'quality' => 'auto',
                                'fetch_format' => 'auto'
        ...
        if ($request->hasFile('comments_vid_path')) {
            ...
            try {
                ...
                $uploadCloudinary = cloudinary()->upload(
                    $request->file('comments_vid_path')->getRealPath(),
                    [
                        'folder' => 'africtv/inner_comment_videos',
                        'resource_type' => 'auto',
                        'transformation' => [
                            'quality' => 'auto',
                            'fetch_format' => 'auto'
        ...
        if (!$comment) {
            return response()->json([
                "status" => false,
                "message" => "Oops! Comment Not found"
            ]);
        }


        // Create the comment
        $comments = InnerComment::create([
            "post_id" => $post->id,
        ...
        return response()->json([
            "status" => true,
            "message" => "Inner Comment Uploaded Successfully",
            "comment" => $comments
        ]);
    }


        public function deleteInnerComment(Request $request)
        {
            // Validate the request
            $request->validate([
                'comment_id' => 'required',
            ...
            // Get the comment that has this comment_id
            $commentId = $request->input('comment_id');
            $comment = InnerComment::find($commentId);
            ...
            if (Auth::user()->id === $comment->user_id) {
                ...
                return response()->json([
                    "status" => true,
                    "message" => "Inner Comment deleted successfully"
                ]);
            } else {
                return response()->json([
                    "status" => false,
        ...
        public function readInnerComment($commentID)
        {
